...
import type { KtuSemesterContent } from "@/lib/content/ktu-semester";

interface KtuSemesterPageProps {
  content: KtuSemesterContent;
  breadcrumbItems: Array<{ name: string; url: string }>;
}

const resourceIcon = (title: string) => {
  if (title.toLowerCase().includes("note")) return <BookOpen className="h-5 w-5" />;
  if (title.toLowerCase().includes("paper") || title.toLowerCase().includes("question"))
    return <FileText className="h-5 w-5" />;
  if (title.toLowerCase().includes("demo")) return <PlayCircle className="h-5 w-5" />;
  return <Tv className="h-5 w-5" />;
};

export function KtuSemesterPage({ content, breadcrumbItems }: KtuSemesterPageProps) {
  ...
  return (
    <div className="bg-white pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative">
            <div className="mb-7">
              ...
              <h2 className="text-2xl font-bold leading-tight text-white">
                {content.subjectsHeading}
              </h2>
              <p className="mt-2 text-sm text-white/60">{content.subjectsIntro}</p>
            </div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {content.subjects.map((subject) => (
                <div
                  key={subject.code}
                  className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-4 transition-all duration-300 hover:border-brand-orange/30 hover:bg-white/[0.07]"
                >
                  <div className="flex h-12 w-16 shrink-0 flex-col items-center justify-center rounded-xl bg-brand-orange/15">
                    <span className="text-xs font-extrabold leading-none text-brand-orange">
                      {subject.code.replace(/\D/g, "")}
                    </span>
                    <span className="mt-0.5 text-[10px] font-semibold uppercase leading-none text-brand-orange/70">
                      {subject.code.replace(/\d/g, "")}
                    </span>
                  </div>
        ...
        <section className="mb-12 rounded-3xl border border-gray-100 bg-white p-7 md:p-9 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {content.resources.map((resource) => (
              <div
                key={resource.title}
                className="flex flex-col gap-4 rounded-2xl border border-gray-100 bg-gray-50/60 p-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-brand-orange/30 hover:bg-white hover:shadow-md"
              >
                ...
                <Link
                  href={resource.href}
                  className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-orange transition-colors hover:text-brand-orange-dark"
                >
                  {resource.linkLabel}
        ...
        <section className="relative mb-12 overflow-hidden rounded-3xl border border-white/10 bg-charcoal p-7 md:p-10 shadow-[0_30px_60px_-40px_rgba(0,0,0,0.8)]">
          <div className="pointer-events-none absolute inset-0">
            <div className="absolute -left-16 top-0 h-64 w-64 rounded-full bg-indigo-500/10 blur-3xl" />
            <div className="absolute -right-10 bottom-0 h-64 w-64 rounded-full bg-brand-orange/10 blur-3xl" />
          </div>
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {content.prepBullets.map((bullet) => (
                <div
                  key={bullet}
              ...
              <div className="mt-6 rounded-2xl border border-indigo-500/20 bg-indigo-500/[0.06] p-5">
                <div className="mb-2 flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-wide text-indigo-300">
                    {content.gateNote.heading}
                  </span>
                </div>
                <p className="mb-2 text-sm leading-relaxed text-white/80">{content.gateNote.note}</p>
                <p className="text-xs leading-relaxed text-white/50">{content.gateNote.footer}</p>
              </div>
            )}
          </div>
        </section>
        ...
        {content.internalLinks.length > 0 && (
          <section className="relative mb-12 overflow-hidden rounded-3xl border border-brand-orange/20 bg-gradient-to-br from-brand-orange/5 via-white to-indigo-50/50 p-7 md:p-9 shadow-sm">
            ...
            <p className="mb-5 text-sm text-gray-600">
              Continue exploring KTU resources and other semesters.
            </p>
            ...
            <div>
              ...
              <p className="text-sm leading-relaxed text-white/70 sm:text-base">
                {content.ctaDescription ??
                  "One-to-one coaching is available for all subjects. Call or WhatsApp to get a personalised study plan."}
              </p>
            </div>
            ...
            <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  Common questions about KTU {content.heroBadge ?? "semester"} resources and coaching.
                </p>
              </div>
            </div>
