      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-12 lg:gap-16 items-start">
          <div ref={headingRef} className="lg:sticky lg:top-28">
            ...
            >
              {content.heading} <span className="text-brand-orange">{content.headingHighlight}</span>
            </h2>

            <div className="faq-heading-line rounded-2xl bg-white/[0.04] backdrop-blur-sm border border-white/[0.08] p-6">
              <h3 className="text-lg font-semibold text-white mb-2">{content.ctaHeading}</h3>
              <p className="text-sm leading-relaxed text-white/50 mb-5">{content.ctaDescription}</p>
              <a
                href={whatsappUrl}
                target="_blank"
          ...
          <div className="faq-list space-y-3">
            {content.items.map((item, i) => {
              ...
              return (
                <motion.div key={i} className="faq-item" initial={false}>
                  <button
                    onClick={() => toggle(i)}
