...
interface KtuSupplyExamPageProps {
  content: KtuSupplyExamContent;
  breadcrumbLabel?: string;
  breadcrumbUrl?: string;
}

export function KtuSupplyExamPage({
  content,
  breadcrumbLabel,
  breadcrumbUrl,
}: KtuSupplyExamPageProps) {
  const breadcrumbItems = [
    { name: "KTU", url: "/ktu" },
    {
      name: breadcrumbLabel ?? "Supply Exam",
      url: breadcrumbUrl ?? "/ktu/supply-exam",
    },
  ];
  ...
  return (
    <div className="bg-white pt-24 pb-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative grid grid-cols-1 items-start gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <Badge variant="violet" className="mb-4 !bg-brand-orange/15 !text-brand-orange">
                {content.heroBadge ?? "Supply Exam Coaching"}
              </Badge>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 lg:grid-cols-1">
              {[
                {
                  title: "All Subjects",
                  subtitle: "Every KTU branch covered across all semesters",
                  icon: <BookOpen className="h-4 w-4 text-brand-orange" />,
                },
                {
                  title: "Both Schemes",
                  subtitle: "2019 and 2024 scheme supply exam coaching",
                  icon: <Layers className="h-4 w-4 text-brand-orange" />,
                },
                {
                  title: "Targeted Prep",
                  subtitle: "Focused on what matters most for your exam",
        ...
        {/* What is a KTU Supply Exam? */}
        <section className="mb-12 rounded-3xl border border-gray-100 bg-white p-7 md:p-9 shadow-sm">
          <div className="mb-5 flex items-center gap-3">
            <span className="inline-flex h-9 w-9 items-center justify-center rounded-xl bg-brand-orange/10 text-brand-orange">
              <BookOpen className="h-4 w-4" />
            </span>
            <h2 className="text-xl font-semibold text-gray-900">{content.whatIsHeading}</h2>
          </div>
          {content.whatIsIntro && (
            <p className="mb-5 border-l-4 border-brand-orange/70 pl-4 text-base leading-8 text-gray-700">
              {content.whatIsIntro}
            </p>
          )}
          <ul className="space-y-3">
            {content.whatIsBullets.map((bullet) => (
        ...
        {/* Why Supply Exam Preparation is Different */}
          ...
          <div className="relative">
            <div className="mb-7">
              ...
              <h2 className="text-2xl font-bold leading-tight text-white">
                {content.whyDifferentHeading}
              </h2>
            </div>

            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {content.whyDifferentBullets.map((bullet) => (
                <div
                  key={bullet}
        ...
        <section className="mb-12 rounded-3xl border border-gray-100 bg-white p-7 md:p-9 shadow-sm">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 pb-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-semibold text-gray-900">{content.howWeHelpHeading}</h2>
            </div>
            <span className="inline-flex items-center rounded-full bg-brand-orange/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-brand-orange">
              Step by Step
            </span>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {content.howWeHelpSteps.map((step, index) => (
              <div
                key={step.title}
                className={`flex items-start gap-4 rounded-2xl border border-gray-100 bg-gray-50/60 p-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-brand-orange/30 hover:bg-white hover:shadow-md ${
                  index === content.howWeHelpSteps.length - 1 &&
                  content.howWeHelpSteps.length % 2 !== 0
                    ? "md:col-span-2"
                    : ""
                }`}
              >
            ...
            <div>
              ...
              <p className="text-sm leading-relaxed text-white/70 sm:text-base">
                {content.ctaDescription ??
                  "Tell us your supply subject, semester, and exam date. We will build a focused preparation plan and start immediately."}
              </p>
            </div>
            ...
            <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  Common questions about KTU supply exam preparation and coaching.
                </p>
              </div>
            </div>
