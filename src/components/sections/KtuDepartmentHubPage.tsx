...
import type { KtuDepartmentHubContent } from "@/lib/content/ktu-department-hub";

interface KtuDepartmentHubPageProps {
  content: KtuDepartmentHubContent;
  breadcrumbItems: Array<{ name: string; url: string }>;
}

export function KtuDepartmentHubPage({
  content,
  breadcrumbItems,
}: KtuDepartmentHubPageProps) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative grid grid-cols-1 items-start gap-8 lg:grid-cols-[1.1fr_0.9fr]">
            <div>
              <Badge variant="violet" className="mb-4 !bg-brand-orange/15 !text-brand-orange">
                {content.heroBadge ?? "Department Hub"}
              </Badge>
        ...
        <section className="mb-10 rounded-3xl border border-gray-100 bg-white p-7 md:p-9 shadow-sm">
          <div className="mb-5 flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">{content.aboutHeading}</h2>
          </div>
          ...
          <div className="relative">
            <div className="mb-7">
              ...
              <h2 className="text-2xl font-bold leading-tight text-white">
                {content.semesterHeading}
              </h2>
              <p className="mt-2 text-sm text-white/60">{content.semesterIntro}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {content.semesters.map((sem) => (
                <Link
                  key={sem.code}
                  href={sem.href}
                  className="group flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] p-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-brand-orange/50 hover:bg-white/[0.08]"
                >
                  <div>
                    <p className="text-xl font-extrabold text-brand-orange">{sem.code}</p>
                    <p className="text-xs text-white/60">{sem.label}</p>
                  </div>
                  <ChevronRight className="h-4 w-4 text-white/30 transition-colors group-hover:text-brand-orange" />
                </Link>
              ))}
            </div>
          </div>
        ...
        <section className="mb-12 rounded-3xl border border-gray-100 bg-white p-7 md:p-9 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {content.offerSteps.map((step, index) => (
              <div
                key={step.title}
                className={`flex items-start gap-4 rounded-2xl border border-gray-100 bg-gray-50/60 p-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-brand-orange/30 hover:bg-white hover:shadow-md ${
                  index === content.offerSteps.length - 1 &&
                  content.offerSteps.length % 2 !== 0
                    ? "md:col-span-2"
                    : ""
                }`}
              >
                <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-brand-orange text-sm font-bold text-white shadow-sm">
                  {index + 1}
                </span>
                <div>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {content.subjects.map((subject) => (
                ...
                >
                  ...
                  <p className="text-sm leading-relaxed text-white/75">{subject}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {content.internalLinks.length > 0 && (
          <section className="relative mb-12 overflow-hidden rounded-3xl border border-brand-orange/20 bg-gradient-to-br from-brand-orange/5 via-white to-indigo-50/50 p-7 md:p-9 shadow-sm">
            ...
            <p className="mb-5 text-sm text-gray-600">
              Continue exploring KTU resources, schemes, and coaching options.
            </p>
            ...
            <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-sm text-gray-500">
                  Common questions about KTU {content.heroBadge ?? "department"} resources and coaching.
                </p>
              </div>
            </div>
